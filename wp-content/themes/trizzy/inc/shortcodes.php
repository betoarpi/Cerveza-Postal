...
$output .= '
    <div class="parallax-banner fullwidth-element '.$custom_class.'"  data-background="'.$background.'" data-opacity="'.esc_attr($opacity).'" data-height="'.esc_attr($height).'">
        ...
        <div class="parallax-title">'.$title.'<span>'.$subtitle.'</span></div>
    </div>';
if($from_vs === 'yes') {
    $output .= '
<div class="container">
            ...
            $output .= '
                <div class="'.$mainclass.' '.$colclass.' recent-blog">
                    ...
                    <section class="from-the-blog-content">
                        ...
                        <i>'.__('By','trizzy'). ' <a class="author-link" itemprop="url" rel="author" href="'.get_author_posts_url(get_the_author_meta('ID',$author_id )).'">'.get_the_author_meta('display_name',$author_id).'</a>'.' '.__('on','trizzy').' '.get_the_date().'</i>
                        <span>';
                            $excerpt = get_the_excerpt();
                            $output .= string_limit_words($excerpt,10);
                    $output .= '</span>
...
function trizzy_info_banner( $atts, $content ) {
   extract(shortcode_atts(array(
    ...
    <div class="info-banner">
        ...
        <a href="'.$url.'" class="button color">'.$buttontext.'</a>
        <div class="clearfix"></div>
    </div>';
    return $output;
}
